...
public class OperationParenthesis {
    ...
    private int searchEndParenthesis(String entrada) {
        int cont = entrada.indexOf(Constans.PARENTHESIS_START);
        boolean finish = false;
        int parenthesisStart = CONTADOR_START_PARENTHESIS;
        int parenthesisEnd = CONTADOR_END_PARENTHESIS;
        while(!finish){
            cont++;
            String character = entrada.substring(cont,cont+1);
    ...
    private int searchStartParenthesis(String entrada) {
        return entrada.indexOf(Constans.PARENTHESIS_START);
    }
}
