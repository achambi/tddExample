    ...
    public String result(){
        ...
        while(!finish){
            ...
            if(Constans.OPERATOR_MULTIPLY.equals(character) || Constans.OPERATOR_DIVIDE.equals(character)){
                ...
                int endPositiion = searchEndPositionOperation(positionOperator, this.input);
                OperationSimple operation = new OperationSimple(this.input.substring(startPosition,endPositiion));
                String resultOperation = operation.result();
                result = this.input.substring(0,startPosition) + resultOperation + this.input.substring(endPositiion);
                finish = true;
            }
            positionOperator++;
        }
